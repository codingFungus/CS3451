...
Light lights[3];                        //// two lights
Sphere spheres[6];                      //// three spheres
Plane planes[1];                        //// one plane
Material materials[5];                  //// four materials: 0 - ground, 1-3 - the three spheres
...
void initScene() 
{
    camera = Camera(vec3(10, 40, 90), vec3(5,0,0), vec3(0, 3, -2), vec3(-2.5, -1.5, -1));

    // Floor Material 
    materials[0].ka = vec3(0.1);
    materials[0].kd = vec3(0.5725, 0.6353, 0.9451);
    materials[0].ks = vec3(0.8667, 0.7294, 0.8549);
    ...
    materials[4].ka = vec3(0.2);
    materials[4].kd = vec3(0.4745, 0.0902, 0.8824);
    ...
    lights[1] = Light(vec3(0.5, 2, 1), 
                            ...
                            /*Id*/ vec3(0.9, 0.9, 0.9), 
                            /*Is*/ vec3(0.5, 0.5, 0.5));
    lights[2] = Light(vec3(1, 4, -1), 
                            /*Ia*/ vec3(0.1, 0.1, 0.1), 
                            /*Id*/ vec3(0.6549, 0.9137, 0.6196), 
                            /*Is*/ vec3(0.8863, 0.7373, 0.498));
    ...
    spheres[3] = Sphere(vec3(-0.6, 0.4, 0.5), 0.4, 0);
    spheres[4] = Sphere(vec3(0.5, 0.2, 1.0), 0.2, 4);
    spheres[5] = Sphere(vec3(-1.1, 0.3, 1.1), 0.3, 2);
}
...
vec3 sampleDiffuse(int matId, vec3 p) 
{
    ...
    if(matId == 0) {		
        ...
        /* your implementation starts */
        vec3 textureColor = texture(floor_color, uv).rgb;

        color = textureColor * mat_color;
...
bool isShadowed(Light light, Hit h) 
{
    ...
    /* your implementation starts */
    vec3 shadowRayOrigin = intersect + Epsilon * dir;
    Ray shadowRay = Ray(shadowRayOrigin, dir);


    Hit shadowHit = findHit(shadowRay);
    if (shadowHit.t > Epsilon && shadowHit.t < t_max) {
        // There is an object between the point and the light source, casting a shadow
        return true;
    }
...
void main() 
{
    ...
    for(int i = 0; i < recursiveDepth; i++) {
        ...
        vec3 reflected_dir = vec3(0);
		/* your implementation starts */
        reflected_dir = normalize(reflect(incoming_dir, normal));
