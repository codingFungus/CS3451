...
const Light light4 = Light(/*position*/ vec3(-5, -3, 3), 
                            /*Ia*/ vec3(0.1, 0.1, 0.1), 
                            /*Id*/ vec3(1.0, 0.7, 0.7), 
                            /*Is*/ vec3(0.5, 0.5, 0.5));
...
vec4 shading_texture_with_checkerboard() 
{
    ...
    float checker = mod(floor(uv.x * checkerSize) + floor(uv.y * checkerSize), 2.0);
    if (checker < 1.0) {
        color = vec3(1.0); // White color for one grid cell
    } else {
        color = vec3(0.0); // Black color for the adjacent grid cell
    }
...
vec4 shading_texture_with_phong(Light light, vec3 e, vec3 p, vec3 s, vec3 n) 
{
    ...
    vec3 diffuse = kd * light.Id * diffuse_factor * tex_color;
...
vec3 read_normal_texture() 
{
    ...
    /* your implementation starts */
    vec3 encoded_normal = texture(tex_normal, uv).rgb;
...
vec4 shading_texture_with_normal_mapping()
{
    ...
    vec3 normal_from_texture = read_normal_texture();
    perturbed_normal = calc_perturbed_normal(TBN, normal_from_texture);
...
vec4 custom_texture()
{
    ...
    vec4 color = shading_texture_with_normal_mapping() + shading_texture_with_phong(light4, e, p, light4.position, perturbed_normal);
